...
func SetWIPLimits(store *storage.FeatureStorage) ToolHandler {
	return func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
		...
		return helpers.JSONResult(map[string]any{
			...
			"message":         fmt.Sprintf("Set WIP limit to %d for project %s", maxInProgress, projectID),
		})
	}
}
	...
	return func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
		...
		if err != nil {
			...
			return helpers.JSONResult(map[string]any{
				"project_id":      projectID,
				"max_in_progress": 0,
				"message":         "No WIP limits configured for this project",
			})
		}

		return helpers.JSONResult(map[string]any{
			"project_id":      projectID,
			"max_in_progress": limit,
		})
	}
}
...
func CheckWIPLimit(store *storage.FeatureStorage) ToolHandler {
	return func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
		...
		limit, err := wipStore.Get(ctx, projectID)
		if err != nil {
			// No WIP limit set, always allowed.
			return helpers.JSONResult(map[string]any{
				"project_id":      projectID,
				"max_in_progress": 0,
				"current":         0,
				"within_limit":    true,
				"message":         "No WIP limits configured",
			})
		}
		...
		return helpers.JSONResult(map[string]any{
			...
			"message":         fmt.Sprintf("WIP: %d/%d in-progress", current, limit),
		})
	}
}
