...
func AddDependency(store *storage.FeatureStorage) ToolHandler {
	return func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
		...
		return helpers.JSONResult(map[string]any{
			...
			"message":       fmt.Sprintf("Added dependency: %s depends on %s", featureID, dependsOnID),
		})
	}
}
...
func RemoveDependency(store *storage.FeatureStorage) ToolHandler {
	return func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
		...
		return helpers.JSONResult(map[string]any{
			"feature_id":    featureID,
			"depends_on_id": dependsOnID,
			"message":       fmt.Sprintf("Removed dependency: %s no longer depends on %s", featureID, dependsOnID),
		})
	}
}
...
func GetDependencyGraph(store *storage.FeatureStorage) ToolHandler {
	return func(ctx context.Context, req *pluginv1.ToolRequest) (*pluginv1.ToolResponse, error) {
		...
		var edges []depEdge
		featureMap := make(map[string]*types.FeatureData)

		for _, f := range features {
			featureMap[f.ID] = f
			for _, dep := range f.DependsOn {
				edges = append(edges, depEdge{From: f.ID, To: dep})
			}
		}
